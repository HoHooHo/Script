...
def pack_plist(path, dir, output_path):
    ...
    command_png = 'TexturePacker --format cocos2d --data ' + pngPlist + ' --ignore-files *pvr.ccz --texture-format png --sheet ' + tempPNGName + ' --algorithm MaxRects --maxrects-heuristics Best --basic-sort-by Best --trim-mode Trim --opt RGBA8888 --max-width 2048 --max-height 2048 --size-constraints NPOT  --border-padding 2 --shape-padding 2 --dither-fs  ' + copyToTempDir(path)
...
#    path = os.getcwd()
    ...
    key_options = config.options('tinykey')
    for key in key_options:
        tinykey.append(config.get('tinykey', key))
